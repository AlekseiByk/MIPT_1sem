...
struct phrase_t{
	...
	{"немнго посокращая получается "},
	{"очевидно, что "},
	{"по формуле производной сложной функции "},
	{"спустя N преобразований"},
	{"возбмем за лемму 1"},
	{"преобразовывая предыдущее получаем"},
	{"хопа"},
	{"немного напрягая мозг, понимаем, что это работает"},
	{"из этого получаем"}
};
...
void GraphPrintTree(FILE *dump, tree_node_t* headNode)
{
	...
	if (headNode -> data.type == TYPENUMBER)
    	fprintf(dump, "\"%p\"[label = \"{<data> %lg}\", color = \"red\"];\n", headNode, headNode -> data.value);
...
void GraphTreeDump(tree_node_t* headNode)
{
    ...
    char FileIn[17] = "";
    snprintf(FileIn, sizeof(FileIn), "dumps/Dump%02d.dot", DumpNumber);
    ...
    Systemf("dot -Tpdf  dumps/Dump%02d.dot -o dumps/Dump%02d.pdf", DumpNumber, DumpNumber);
    Systemf("rm dumps/Dump%02d.dot", DumpNumber);
...
#define MULMULOPT(node1, node2, node3, op1, op2)													\
		if (IfFunc(node, op1) && IfFunc(node1, op2) && IfNumber(node3) && IfNumber(node2) )			\
		{																							\
...
#define nrl node -> right -> left
#define nrr node -> right -> right
#define nll node -> left -> left
#define nlr node -> left -> right

tree_node_t* treeNodeOptimize(tree_node_t* node, int* counter)
{
	assert(node);

	if ((nl != nullptr) && (nl -> data.type != TYPENUMBER))
		nl = treeNodeOptimize(nl, counter);
	if ((nr != nullptr) && (nr -> data.type != TYPENUMBER))
		nr= treeNodeOptimize(nr, counter);

	if (IfFunc(node, '*') && (IfNum(nl, 0) || IfNum(nr, 0)))
	{
		treeDestruct(node);
	...
	OPTFUNCNUMBER(right, left, '-', 0);
	OPTFUNCNUMBER(right, left, '^', 1);
	if ((node -> data.type == TYPEFUNCTION) && (!(nl) || (nl -> data.type == TYPENUMBER)) && (nr -> data.type == TYPENUMBER))
	{
		(*counter)++;
	...
	DIVDIVOPT(nl, nr, nlr, '/', '/');
	DIVDIVOPT(nr, nl, nrr, '/', '/');
	if (IfFunc(node, '*') && IfFunc(nl, '/') && IfNumber(nr) && IfNumber(nlr) )
		{
			double a = nr -> data.value, b = nlr -> data.value;
			nlr -> data.value = b / a;
			nr -> data.value = 1;
			(*counter)++;
			return node;
		}
	if (IfFunc(node, '/') && IfFunc(nl, '*') && IfNumber(nr) && IfNumber(node->left->right) )
		{
			double a = nr -> data.value, b = nlr -> data.value;
			nr -> data.value = a / b;
			nlr -> data.value = 1;
...
tree_node_t* treeOptimize(tree_node_t* node)
{
    ...
    while (k)
    {
    	LatexWriteText("Latex", phrases[rand() % 10].text);
    	LatexWrite("Latex", node, "");
    	k = 0;
...
void LatexWriteNode(FILE* dump, tree_node_t* headNode)
{
    ...
    if (headNode -> data.type == TYPENUMBER)
    	if (headNode -> data.value == (int) headNode -> data.value)
        	...
        	fprintf(dump, "(%lg)", headNode -> data.value);
    else if (headNode -> data.type == TYPEVARIABLE)
...
void LatexWriteEnd(const char* file)
{
    ...
    Systemf("rm -rf %s.log", file);
    Systemf("rm -rf %s.aux", file); 
    Systemf("rm -rf %s.synctex.gz", file); 
    Systemf("rm -rf %s.tex", file); 
